...
#HEALTHCHECK CMD /bin/bash -c "[[ $(pgrep -lf "Linux/SquadGameServer") > /dev/null ]] && exit 1 || exit 0"
HEALTHCHECK CMD-SHELL if [ $(pgrep -lf "Linux/SquadGameServer") > /dev/null ]; then true; else false; fi
